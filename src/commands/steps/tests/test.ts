...
export async function runJUnitTests(testFile: string, outputDir: string): Promise<boolean> {
    try {
        ...
        const output = await $`java -cp ${outputDir};${junitLibsDir}/* org.junit.platform.console.ConsoleLauncher --class-path ${outputDir} --select-class ${testFile}`.stderr(Deno.stdout).text();
